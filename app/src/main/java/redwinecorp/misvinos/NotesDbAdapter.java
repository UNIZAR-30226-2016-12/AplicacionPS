...
public class NotesDbAdapter {
    ...
    private static final String DATABASE_CREATE =
            "create table notes (_id integer primary key autoincrement, "
                    + "title text not null, body text not null, name text default null, foreign key" +
                    "(name) references category(name));";
    private static final String DATABASE_CREATE2 =
            "create table category (_id integer primary key autoincrement, "
                    + "name text unique not null);";
    private static final String TRIGGER_DB_UPDATE =
            "CREATE TRIGGER actualizado\n" +
                    "BEFORE UPDATE ON category BEGIN " +
                    "UPDATE notes SET name = new.name WHERE name = old.name; " +
                    "END;";
    private static final String TRIGGER_DB_DELETE =
            "CREATE TRIGGER borrado\n" +
                    "BEFORE DELETE ON category BEGIN " +
                    "UPDATE notes SET name = null WHERE name = old.name; " +
                    "END;";
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            ...
            db.execSQL(TRIGGER_DB_UPDATE);
            db.execSQL(TRIGGER_DB_DELETE);
        }
